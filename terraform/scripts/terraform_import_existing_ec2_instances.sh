...
instanceId=$(aws ec2 describe-instances --filter Name=tag:App,Values=speedl --query "Reservations[*].Instances[?!contains(State.Name, 'terminated')].{Instance:InstanceId}" --output text 2>&1)
